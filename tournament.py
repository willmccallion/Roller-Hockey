...
class Tournament:
    ...
    def _save(self):
        s = "rank,name,w,l,ot_l,gp,pts,gf,ga,plus_minus\n"
        rank = 1
        for team in self.all_teams:
            s += f"{rank},{team.get_name()},{team.get_wins()},{team.get_losses()},{team.get_ot_losses()},{team.get_gp()},{team.get_points()},{team.get_gf()},{team.get_ga()}\n"
            rank += 1
...
class Team:
    ...
    def __str__(self) -> str:
        if self.gp > 0:
            return f"Rank: {self.rank:2} | Name: {self.name:>10} |  Record: {self.record[0]}/{self.record[1]}/{self.record[2]} | GP: {self.gp:>2} | PTS: {self.pts} | P%: {(self.pts/2)/self.gp:.2f} | GF: {self.gf} | GA: {self.ga} | PM: {self.gf-self.ga:>3} |"
        else:
            return f"Rank: {self.rank:2} | Name: {self.name:>10} |  Record: {self.record[0]}/{self.record[1]}/{self.record[2]} | GP: {self.gp:>2} | PTS: {self.pts} | P%: 0.00 | GF: {self.gf} | GA: {self.ga} | PM: {self.gf-self.ga:>3} |"